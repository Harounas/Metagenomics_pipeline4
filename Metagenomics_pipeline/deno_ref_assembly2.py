...
def get_best_reference(sample_r1, sample_r2, reference_list):
    ...
    for fasta in reference_list:
        ...
        bwa_command = ["bwa", "mem", fasta, sample_r1, sample_r2]
        try:
...
def download_and_index_reference(tax, scientific_name, tax_dir, skip_existing=False):
    """
    Download and index the reference genome for a given taxonomic ID.
    If skip_existing is True and the reference file already exists, skip download.
    """
    fasta_file = os.path.join(tax_dir, f"{scientific_name}.fasta")
    if skip_existing and os.path.exists(fasta_file):
        logging.info(f"Reference genome for {scientific_name} already exists. Skipping download and indexing.")
        return fasta_file
...
def run_de_novo_assembly(sample, sample_r1, sample_r2, output_dir, skip_existing=False):
    """
    Run de novo assembly using MetaSPAdes.
    If skip_existing is True and the contigs file already exists, skip assembly.
    """
    contigs_file = os.path.join(f"{output_dir}/{sample}_denovo", "contigs.fasta")
    if skip_existing and os.path.exists(contigs_file):
        logging.info(f"De novo assembly already exists for {sample}. Skipping assembly.")
        return contigs_file
...
def deno_ref_based(df, input_dir, output_dir, run_bowtie, skip_existing=False):
    ...
    for tax in taxids:
        ...
        fasta_file = download_and_index_reference(tax, scientific_name, tax_dir, skip_existing=skip_existing)
        if not fasta_file:
        ...
        for sample in dftax['SampleID']:
            try:
                ...
                # Run de novo assembly with skip_existing check
                contigs_file = run_de_novo_assembly(sample, sample_r1, sample_r2, output_dir, skip_existing=skip_existing)
                if not contigs_file:
                    logging.error(f"Skipping {sample} due to de novo assembly failure.")
                    continue

                # Get best reference from pre-split reference genome
                temp_dir = os.path.join(output_dir, f"{sample}_{scientific_name}")
                os.makedirs(temp_dir, exist_ok=True)
                reference_list = split_fasta(fasta_file, temp_dir)
                fasta_file_best = get_best_reference(sample_r1, sample_r2, reference_list)
                if not fasta_file_best:
                    logging.error(f"Skipping {sample}: Unable to determine best reference.")
                    continue

                # Extract accession number from best reference
                cmd = f"grep '^>' {fasta_file_best} | cut -d ' ' -f1 | sed 's/^>//'"
                ...
                # Mapping & Variant Calling with skip_existing checks
                ...
                if not (skip_existing and os.path.exists(bam_file)):
                    ...
                    logging.info(f"BAM file {bam_file} already exists. Skipping mapping.")
                ...
                if not (skip_existing and os.path.exists(vcf_file)):
                    ...
                    logging.info(f"VCF file {vcf_file} already exists. Skipping variant calling.")
                ...
                ref_len = calculate_length(fasta_file_best)
