            ...
            if file_name.endswith("_report.txt"):
                with open(os.path.join(kraken_dir, file_name), 'r') as f:
                    for line in f:
                        ...
                        #if (rank_code == 'S' or rank_code == 'S1' or rank_code == 'S2' or rank_code == 'S3') and nr_frag_direct_at_taxon >= read_count and (nr_frag_direct_at_taxon <=max_read_count if nr_frag_direct_at_taxon is not None):
                        if rank_code in ['S', 'S1', 'S2', 'S3'] and nr_frag_direct_at_taxon >= read_count and (max_read_count is None or nr_frag_direct_at_taxon <= max_read_count):
    ...
    try:
        ...
        for focus, filter_str, plot_title in [
            ...
            if focus == 'Bacteria_Type':
                dffma_focus = df[~df['Scientific_name'].str.contains(filter_str, case=False, na=False)]
            else:
