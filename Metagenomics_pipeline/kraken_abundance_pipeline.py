...
def extract_domains_from_kraken_report(kraken_report_path):
    """
    Extract Bacteria, Eukaryota, Archaea, and Viruses from a Kraken2 report.
    
    Args:
        kraken_report_path (str): Path to the Kraken2 report file.
    
    Returns:
        dict: A dictionary containing DataFrames for each domain.
    """
    # Load Kraken2 report
    columns = [
    ...
    for _, row in df.iterrows():
        # Check if the row is a domain (Rank_Code == 'D')
        if row["Rank_Code"] == "D":
            # Save the current domain's rows
            if current_domain:
            ...
            # Add rows to the current domain
            current_rows.append(row)
    
    # Save the last domain's rows
    if current_domain:
        domains[current_domain] = pd.DataFrame(current_rows)
...
def process_kraken_reports(kraken_dir):
    """
    Process all Kraken2 report files in a directory and save domain-specific results.
    
    Args:
        kraken_dir (str): Directory containing Kraken2 report files.
    """
    # Iterate over each Kraken report file
    for file_name in os.listdir(kraken_dir):
        if file_name.endswith("_report.txt"):
            # Extract the base name (e.g., "sample" from "sample_report.txt")
            parts = file_name.split('_')
            extracted_part = '_'.join(parts[:-2])  # Remove "_report.txt"
            ...
            # Save each domain's data to a separate file
            for domain, df in domains.items():
                output_path = os.path.join(kraken_dir, f"{extracted_part}_{domain.replace(' ', '')}_kraken_report.txt")
                df.to_csv(output_path, sep="\t", index=False)
                print(f"Saved {domain} data to {output_path}")
...
def generate_unfiltered_merged_tsv(kraken_dir, metadata_file=None, sample_id_df=None):
    ...
    try:
        ...
        # Dictionary to store unfiltered results
        unfiltered_results = {}
            ...
            if file_name.endswith("_report.txt"):
                with open(os.path.join(kraken_dir, file_name), 'r') as f:
                    for line in f:
                        ...
                        # Include all ranks and read counts without filtering
                        if extracted_part in metadata[sample_id_col].unique():
                            sample_metadata = metadata.loc[metadata[sample_id_col] == extracted_part].iloc[0].to_dict()
        ...
        # Output unfiltered results to a TSV file
        merged_tsv_path = os.path.join(kraken_dir, "merged_kraken_all_ranks_unfiltered.tsv")
        with open(merged_tsv_path, 'w') as f:
            headers = ['Perc_frag_cover', 'Nr_frag_cover', 'Nr_frag_direct_at_taxon', 'Rank_code', 'NCBI_ID', 'Scientific_name', 'SampleID'] + metadata.columns[1:].tolist()
            f.write("\t".join(headers) + "\n")
            for sampleandtaxonid, data in unfiltered_results.items():
                f.write("\t".join(str(data[col]) for col in headers) + "\n")
        
        print(f"Unfiltered merged Kraken results saved to {merged_tsv_path}")
        return merged_tsv_path
    
    except Exception as e:
        print(f"Error generating unfiltered merged TSV: {e}")
        return None
...
def generate_abundance_plots(merged_tsv_path, top_N, col_filter, pat_to_keep, rank_code='S'):
    ...
    try:
        ...
        for focus, filter_str, plot_title in [
            (f'Virus_{rank_titles[rank_code]}', 'Virus' if rank_code in ['S', 'S1', 'S2'] else 'viridae' if rank_code in ['F', 'F1', 'F2'] else 'virus' if rank_code in ['G', 'G1', 'G2'] else None, f'Viral_{rank_titles[rank_code]}'),
            (f'Bacteria_{rank_titles[rank_code]}', 'Virus' if rank_code in ['S', 'S1', 'S2'] else 'viridae' if rank_code in ['F', 'F1', 'F2'] else 'bacteria' if rank_code in ['G', 'G1', 'G2'] else None, f'Bacterial_{rank_titles[rank_code]}')
        ]:
...
def process_all_ranks(kraken_dir, metadata_file=None, sample_id_df=None, read_count=1, max_read_count=1000000000000000000000000000, top_N=None, col_filter=None, pat_to_keep=None):
    """Processes Kraken results, generates abundance plots for all rank codes (S, K, G, F), and creates an unfiltered merged TSV."""
    # Generate unfiltered merged TSV for all ranks
    unfiltered_tsv = generate_unfiltered_merged_tsv(kraken_dir, metadata_file, sample_id_df)
    
    # Process filtered results and generate plots for specific ranks
    rank_codes = ['S', 'K', 'G', 'F']
